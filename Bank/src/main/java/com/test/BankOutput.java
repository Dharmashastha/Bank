...
public class BankOutput {
	...
	private static AutoGenerate	autoCall=new AutoGenerate();
...
private void customerDetails() throws CustomException
{
	...
	for(int i=1;i <= length;i++)
	{
		...
		long custId=autoCall.addNewCustomerId();
...
private void accountDetails() throws CustomException
{
		...
		long accNo=autoCall.addNewAccountNo();
		account.setAccountNo(accNo);
		long accountNo=account.getAccountNo();
		account.setBalance(autoCall.setMinBalance());
...
{	
	try {
		...
		System.out.println(AutoGenerate.customerId);
		System.out.println(AutoGenerate.accountNo);
		}
		catch(CustomException e)
		{
			e.printStackTrace();
...
case 10:
{
	//CREATE TABLE Login(UserId INT,customerId INT,Password VARCHAR(10),RoleId INT,FOREIGN KEY (customerId) REFERENCES CustomerInfo(customerId),FOREIGN KEY (RoleId) REFERENCES Role(RoleId),PRIMARY KEY(UserId));
